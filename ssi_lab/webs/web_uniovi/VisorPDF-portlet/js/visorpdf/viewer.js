...
'use strict';
//C1NN anyadida esta variable para title de la pagina
var TITLE_DOCUMENT = '';
...
var FindStates = {
  ...
  PDFJS.workerSrc = '/VisorPDF-portlet/js/visorpdf/pdf.js';
...
var PDFView = {
  ...
  setTitle: function pdfViewSetTitle(title) {
	  ...
	  //C1NN Comprobado que no es el de por defecto
	  if(title !='defecto.pdf'){
		 // document.title = title;
		  
		  
		  var mi_url = this.url.split('#')[0];
		  
		   if (window.top === window && mi_url === window.location.href.split('#')[0]) {
		     mi_url += mi_url.indexOf('?') === -1 ? '?' : '&';
		   }
		   mi_url += '#pdfjs.action=download';
		
		   var mi_filename = mi_url.match(/([^\/?#=]+\.pdf)/i);
		
		  TITLE_DOCUMENT = mi_filename ? mi_filename[1] : 'file.pdf';
	  }
  },
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    PDFJS.Promise.all(promises).then(function() {
      ...
      if (window.parent.location === window.location) {
       // C1NN No queremos que vaya a focus
...
document.addEventListener('DOMContentLoaded', function webViewerLoad(evt) {
  ...
  //C1NN Modificado para que coja el lenguage de la pagina web
...
function updateViewarea() {
  ...
  //C1NN Quitada funcion bookmark ya que va por javascript
//  var href = PDFView.getAnchorUrl(pdfOpenParams);
//  document.getElementById('viewBookmark').href = href;
  //  	document.getElementById('viewBookmark').href = DEFAULT_URL;
}
